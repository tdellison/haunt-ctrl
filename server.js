...
async function flashLights(style, allLights) {
  ...
  if (!allLights && ids.length === 0) return;
...
const STRIKE_INTERVAL_MS = 120000;

let strikeIndex  = 0;
let lastStrikeAt = 0; // timestamp of most recent storm strike
...
// ─── Witch cackle timer ───────────────────────────────────────────────────────
async function fireCackle() {
  // If a strike fired in the last 20 seconds, hold off 30s so storm stays dominant
  const stormRecent = Date.now() - lastStrikeAt < 20000;
  if (stormRecent) {
    broadcastLog('Witch cackle: holding 30s (storm just fired)', 'WITCH');
    await new Promise(r => setTimeout(r, 30000));
    if (!state.witchTimer.active) return;
  }

  broadcastLog('Witch cackle', 'WITCH');

  // If storm is running, duck z3 slightly so thunder stays on top
  const duringStorm = state.stormActive;
  const currentZ3   = state.volumes.z3;
  if (duringStorm) {
    const duckedZ3 = clampVol('z3', Math.max(0, currentZ3 - 8));
    try { await sendISCP(`${ZONE_CMD.z3}${volToHex(duckedZ3)}`); } catch (_) {}
  }

  if (fxProcess) { try { fxProcess.kill(); } catch (_) {} fxProcess = null; }
  fxProcess = spawn(VLC_PATH, [
    path.join(AMBIENT_DIR, FX_FILES.cackle),
    '--intf', 'dummy', '--play-and-exit', '--no-video',
  ], { detached: true, stdio: 'ignore' });
  fxProcess.unref();
  fxProcess.on('exit', () => {
    fxProcess = null;
    // Restore z3 after cackle finishes
    if (duringStorm) {
      sendISCP(`${ZONE_CMD.z3}${volToHex(currentZ3)}`).catch(() => {});
    }
  });
}
...
function scheduleNextWitch() {
  if (!state.witchTimer.active) return;
  const ms = 270000 + Math.random() * 60000; // 4.5 – 5.5 minutes
...
app.post('/api/witch/toggle', (req, res) => {
  ...
  broadcastLog(`Witch Cackle Timer ${state.witchTimer.active ? 'ON' : 'OFF'}`, 'WITCH');
