...
function spellLookFor(slot, spellYard) {
  ...
  if (spellYard === 'memory') {
    ...
    return null; // Memory leaves the rest of the yard alone
  }
...
// slots are set once by applyShowScheme; the monument is driven
...
let state = {
  ...
  // Blackout Storm — once per SEASON (season lock lives in seasonBeatsUsed).
  blackout: { running: false, firedAt: null, step: null },
};
...
// ─── State persistence ────────────────────────────────────────────────────────
// Season-level once-only beats. Declared HERE, above saveShowState/loadShowState,
// because loadShowState() runs at startup and touches it - declaring it further
// down left it in the temporal dead zone and silently broke the entire restore.
...
function saveShowState() {
  try {
    fs.writeFileSync(STATE_FILE, JSON.stringify({
      ...
      soundPreset,
      // Season-level once-only beats MUST survive restarts and nights - that is
      // what makes them once-per-SEASON rather than once-per-run.
      seasonBeatsUsed,
    }), 'utf8');
  } catch(_) {}
}
function loadShowState() {
  try {
    ...
    if (saved.seasonBeatsUsed) Object.assign(seasonBeatsUsed, saved.seasonBeatsUsed);
...
const SLOT_LABELS = { skeleton:'Skeleton', witch:'Witches', cauldron:'Cauldron', monument:'Monument' };
...
function tryRollSpecialBeat(beatType, rollFn) {
  ...
  if (fired) {
    ...
    if (SEASON_ONCE_BEATS.includes(beatType)) { seasonBeatsUsed[beatType] = true; saveShowState(); }
  ...
  spells: {
    ...
    memory: {
      ...
      yardLighting: 'Witch lights shift to deep crimson matching the cauldron — two witches connected by ' +
        ...
        'everything; this spell reaches inward not outward; slow fade back to ' +
...
function castSpellLights(spellKey) {
  ...
  track(() => {
    ...
    if (spellKey === 'unraveling') {
      // Snap back: release the yard. Skeleton and witch loops return to base on
      // their next tick.
      effects.spellYard = null;
...
// ─── The Blackout Storm ───────────────────────────────────────────────────────
// ONCE PER SEASON. The storm doesn't build — it takes the yard. Everything
// stops at once, the monument is the last light to die and the first to come
// back, and the four of them speak in the dark before the blast.
//
...
// The moon/ambiance lights are Bluetooth units on a manual smart plug. There is
// no control path from this server yet (the plug's IP/API is not wired up), so
// this is a single seam to fill in rather than a call scattered through the
// sequence. Until then it logs the intent so the sequence still reads correctly
// on the show log and the host can kill the plug by hand if they want to.
function setAmbiancePower(on) {
  // TODO: wire to the ambiance smart plug once its control path is decided.
  broadcastLog(`Ambiance plug: ${on ? 'ON' : 'OFF'} (no control path yet — manual)`, 'LIGHT');
  return false;
}

// No TTS exists yet (see /api/witch/speak). If a cached clip for this beat has
...
function speakBlackoutLine(line) {
  try {
    ...
    if (file) {
      ...
      soundProcess.on('exit',  () => { soundProcess = null; });
      soundProcess.on('error', () => { soundProcess = null; });
      muteMics(BLACKOUT_LINE_GAP_MS, 'blackout line');
      broadcastLog(`Blackout ${line.id}: ${file}`, 'AUDIO');
      return true;
    }
  } catch (_) {}
  broadcastLog(`Blackout ${line.id} (no audio): "${line.text}"`, 'AUDIO');
  return false;
}

function blackoutEligible(force) {
  if (state.blackout.running) return { ok: false, why: 'blackout storm already running' };
  if (!force && seasonBeatsUsed.blackoutStorm) return { ok: false, why: 'already used this season' };
  if (!force && phantomSuppressed) return { ok: false, why: 'Grand Ritual in progress' };
  if (!force && calmActive) return { ok: false, why: 'calm phase in progress' };
  return { ok: true };
}

const sleep = ms => new Promise(r => setTimeout(r, ms));

// force:true skips the season lock and the stage guards — Test tab only.
async function blackoutStorm(force = false) {
  const check = blackoutEligible(force);
  if (!check.ok) return { ok: false, error: check.why };

  const allSlots  = Object.keys(SLOT_BASES);
  const otherIds  = getSlotIds(...allSlots.filter(s => s !== 'monument'));
  const monIds    = getSlotIds('monument');
  const allIds    = getSlotIds(...allSlots);

  state.blackout.running = true;
  state.blackout.firedAt = Date.now();
  if (!force) { seasonBeatsUsed.blackoutStorm = true; saveShowState(); }
  acquireAudioLock('blackout', 'grand_ritual');
  phantomSuppressed = true;          // nothing ambient speaks over this
  effects.suspended = true;          // loops keep ticking but stay silent
  const wasAmbient = ambientSystem.active;
  const step = s => { state.blackout.step = s; broadcastState(); };

  try {
    // 1 — abrupt cutout. Not a fade. Every sound in the yard stops at once.
    step('cutout');
    broadcastLog('BLACKOUT STORM — everything cuts out', 'SYSTEM');
    if (wasAmbient) stopAmbientSystem();
    for (const p of [stormProcess, soundProcess, skeletonProcess, witchProcess]) {
      if (p) { try { p.kill(); } catch (_) {} }
    }
    setMonumentBlackout(true);       // monument holds a bare ember
    if (otherIds.length) await goveeSetBrightness(0, otherIds, 0).catch(() => {});

    // 2 — the monument is the LAST light to go dark.
    await sleep(BLACKOUT_MONUMENT_MS);
    step('dark');
    setMonumentBlackout(false);
    setMonumentOverride(0);          // stage 0 = off
    if (monIds.length) await goveeSetBrightness(0, monIds, 0).catch(() => {});

    // 3 — the laugh. Its own file; the evil-laugh stingers are NOT a substitute.
    await sleep(600);
    step('laugh');
    const laugh = findSoundFile(BLACKOUT_LAUGH_KW);
    if (laugh) playHauntSound(laugh);
    else broadcastLog(`Blackout: no "${BLACKOUT_LAUGH_KW}" file in HAUNT SOUNDS — laugh skipped`, 'AUDIO');

    // 4 — ambiance plug off. The yard is genuinely dark now.
    setAmbiancePower(false);

    // 5 — four lines, fixed order, pauses between.
    await sleep(2600);
    step('lines');
    for (const line of BLACKOUT_LINES) {
      speakBlackoutLine(line);
      broadcast({ type: 'blackout_line', data: line });
      audioLock.since = Date.now();  // this beat outruns the 45s lock watchdog
      await sleep(BLACKOUT_LINE_GAP_MS);
    }

    // 6 — dead air.
    step('silence');
    await sleep(BLACKOUT_SILENCE_MS);

    // 7 — the blast. Every slot, full white, together.
    step('blast');
    audioLock.since = Date.now();
    playStormFile(OVERHEAD_FILE);
    const blastIds = allIds.length ? allIds : undefined;
    if (allIds.length || goveeDevices.length) {
      await goveeSetColor(255, 255, 255, blastIds, 0).catch(() => {});
      await goveeSetBrightness(100, blastIds, 0).catch(() => {});
    }
    await sleep(BLACKOUT_BLAST_MS);

    // 8 — monument first back, then the rest of the yard.
    step('return');
    setMonumentOverride(null);
    setMonumentBlackout(true);       // ember returns ahead of everything else
    if (monIds.length) {
      await goveeSetColor(0, 140, 150, monIds, 0).catch(() => {});
      await goveeSetBrightness(6, monIds, 0).catch(() => {});
    }
    await sleep(1200);
    setMonumentBlackout(false);
    setAmbiancePower(true);
    effects.suspended = false;
    await applyShowScheme().catch(() => {});
    if (wasAmbient) await startAmbientSystem().catch(() => {});
    broadcastLog('Blackout storm complete — yard restored', 'SYSTEM');
    return { ok: true, forced: !!force };
  } finally {
    // Never strand the yard: whatever failed above, the show comes back.
    state.blackout.running = false;
    state.blackout.step = null;
    effects.suspended = false;
    effects.monumentOverride = null;
    effects.monumentBlackout = false;
    phantomSuppressed = false;
    releaseAudioLock('blackout');
    broadcastState();
  }
}
...
// ─── Blackout Storm ───────────────────────────────────────────────────────────
// { force: true } bypasses the once-per-season lock for Test tab verification.
app.post('/api/blackout/fire', async (req, res) => {
  const force = !!(req.body && req.body.force);
  const result = await blackoutStorm(force);
  if (!result.ok) return res.status(409).json(result);
  res.json(result);
});

app.get('/api/blackout/status', (req, res) => {
  const check = blackoutEligible(false);
  res.json({
    ok: true,
    running: state.blackout.running,
    step: state.blackout.step,
    firedAt: state.blackout.firedAt,
    usedThisSeason: !!seasonBeatsUsed.blackoutStorm,
    eligible: check.ok,
    reason: check.ok ? null : check.why,
    laughFile: findSoundFile(BLACKOUT_LAUGH_KW),
    lines: BLACKOUT_LINES,
  });
});
