...
## Hardware
- **Receiver**: Onkyo TX-NR838, ISCP over TCP at `192.168.1.160:60128` (DHCP-reserved, MAC 00:09:b0:9b:de:c3). Zone volume 0–80 hex. A command queue (`queueISCP`) serializes traffic.
...
- **Playback**: **ffplay** (`C:\ffmpeg\ffmpeg-2026-09-10-git-fd7c73d01e-full_build\bin\ffplay.exe`), flags `-nodisp -autoexit -loglevel error`. It REPLACED VLC: VLC's dummy interface takes ~1s to open the audio device, which is the entire length of a one-word character line, so every Blackout line played as a half-second blip while multi-second storm clips were fine. All playback goes through `playerBin()` / `playerArgs()` - one place, never per-call-site argument lists. `PLAYER.kind` switches to **mpv** if per-zone output devices are ever needed (`AUDIO_DEVICES`): ffplay CANNOT select an output device and follows the Windows default, mpv can. Today the Onkyo does the zone splitting and the Dell has one output, so ffplay is correct and `AUDIO_DEVICES` stays empty. Startup prints the player path, or a loud MISSING line - a missing binary otherwise fails silently. The ffmpeg folder name carries its build date, so a future ffmpeg update changes the path and `FFPLAY_PATH` must be updated with it.
...
## Zones (final layout)
- **Zone 1 (z1/MVL) — SKELETONS**: Front L/R + Center terminals. **Jasper = left skeleton = FL speaker, Edgar = right = FR**, center between them. Skeletons ~7–8 ft apart, tucked into island trees. One is white (Edgar), one brown/rotting (Jasper). **Jasper's guest mic was DROPPED (2026 budget decision)** — he reacts to PIR-sensed presence only; Evelina's is the only guest-facing mic this year (`hasMic:false` in CHARACTERS).
...
- **Monument progression** (`MONUMENT_STAGES`, `monumentTick`): indices line up 1:1 with the stage machine — 0 Distant completely OFF → 1 Building barely visible cold white-grey → 2 Active green-teal pulsing → 3 Grand Ritual full spectral green-blue cycling → 4 Calm bare ember (the last thing still lit). It emerges, it does not flicker. `MONUMENT_RITUAL_IDX` is the index the ritual override drives it to.
...
- **Anomaly hooks**: `setMonumentOverride(stageIdx|null)` drives the monument independently of the storm stage (wrong-storm anomaly = go spectral before the incantation). `setMonumentBlackout(true/false)` holds it at a bare ember so it's the last light dark and the first back after the blast. **Blackout Storm is BUILT and tested end-to-end** (`blackoutStorm()`, Test tab button, season lock via `seasonBeatsUsed`, dedicated ghostly-laugh spectral sound that must NEVER play in Grand Ritual — the ritual uses the evil-laugh-1/2/3 stingers); it awaits only a Director to roll it via `tryRollSpecialBeat`. The wrong-storm anomaly still doesn't exist in the storm engine.
...
## Cycle state, special beats, Evelina clue tiers
- **`cycleState.specialBeatFired`** — only ONE special beat per cycle, so two big moments never stack. Reset on entry to Distant (and on every `strikeIndex = 0`).
- **`tryRollSpecialBeat(beatType, rollFn)`** — the Director calls this instead of rolling directly. `SPECIAL_BEATS` = blackoutStorm, lenoraAlmostTruth, nearSuccessCycle, stormEcho.
- **`seasonBeatsUsed`** — SEPARATE season-level once-only store for `SEASON_ONCE_BEATS` (blackoutStorm, lenoraAlmostTruth). The per-cycle mutex never loosens these.
- **Evelina clue tiers** — `evelinaCluesVague` / `evelinaCluesPointed`, picked by `pickEvelinaClue(cycleNumber)`; cycles 1–2 vague, cycle 3+ pointed (`EVELINA_CLUE_POINTED_FROM_CYCLE`), still random WITHIN the tier. **Both pools are intentionally EMPTY** — the owner writes Evelina's lines; sort existing clues in, never invent them. The picker falls back to the other tier rather than returning nothing.
- **None of this is wired yet** — there is no Director agent in the code. These are the structures it will call.

## Storm engine
**Canonical 5-stage cycle**, 2-min intervals, auto-repeats: **Distant → Building → Active → Grand Ritual → Calm After Storm** → reset. **Grand Ritual IS the peak** — there is no separate "Overhead" stage after it; the ritual sequence and the all-lights white blast both happen inside stage 4 (index 3). Calm After Storm is a real stage (index 4): no storm clip, no flash — it hands off to `calmPhase()`, which owns the lights for its 60–90s hold. Random clip from `STORM_FILES` for stages 1–3; **Grand Ritual always plays `646912__alexdarek__lightning-strike-2.wav`** (never randomized) and fires the fog burst. **Fog and lighting run OUTSIDE the receiver try/catch** — an ISCP timeout must never swallow the visual half of a stage. Manual controls live in the Test tab (FIRE STRIKE NOW, per-stage light tests, per-clip audio tests).

## Media folders (LOCAL — `C:\haunt-ctrl-assets\`, moved off OneDrive)
Folders: `storm`, `graveyard-ambient` (note the hyphen), `skeleton`, `witch`, `ambient-sounds` (overlay FX, formerly "Haunt sounds"). Set by `STORM_DIR` / `AMBIENT_DIR` / `SKELETON_DIR` / `WITCH_DIR` / `HAUNT_SOUNDS_DIR`. **All five verified present with every required file.**
- **Phantom sounds resolve 10/10** — files in `ambient-sounds` are named to match the `PHANTOM_MAP` keywords (crow caw, owl hooting, wolf howl, gate creak, evil laugh 1-3, floorboard creak, cat hiss, digging with spade, churchbell, ouch scream). `findSoundFile` does a substring match, so keeping those words in the filename is what keeps them working.
- `AUDIO_EXT` filters both `findSoundFile` and `/api/sounds/list` to real audio, so manifests/notes in the folder are never listed or played.
- Storm: 5 randomized clips + `646912__alexdarek__lightning-strike-2.wav` for the Grand Ritual. Ambient bed: `graveyardam.mp3` (hardcoded, `AMBIENT_FILE`).
- Voices: `skeleton-left.wav`/`skeleton-right.wav`, `witch-main-left.wav` (Evelina) / `witch2-right.wav` (Lenora), stereo hard-panned. `witch1-right.wav` and `witch2-left.wav` also exist but are UNUSED (wrong channel for their character).
- **Projections REMOVED** — rebuild from git history if they ever return. The `WH_*.mp4` files in `witch/` are still used as audio-only clips via `WITCH_MAP`.
- `FX_FILES` soundboard was DELETED — stale filenames, and nothing called it.

## Characters & spells
Show identity: **The Hollow Storm** at **Thornfield Cemetery, Est. 1724** — cemetery predates the ritual by decades; built/managed by the Thorn family before Evelina arrived (Lenora's family connection is never explicitly stated — lore for those who notice). **The tombstone prop is "the unmarked grave"** (its marker = "the monument", Slot 8) — those are the ONLY two terms; the older word for it is retired everywhere. Its monument is carved "Thornfield Cemetery Est. 1724" (cemetery signage only; the occupant is never named). Evelina references Thornfield casually, Lenora with personal weight.
...
- **Jasper Bones** — left skeleton (z1 FL, NO mic — PIR-presence reactive only, 2026 decision): nervous, superstitious; notices every effect first; builds suspense.
...
- **Unmarked grave guardrail is PERCEPTUAL, not behavioural** (in all 4 prompts + `CHARACTER_BIBLE.unmarkedGrave`): the grave does not register to them as significant at all — not mysterious, not unsettling, just an ordinary forgettable headstone they are incapable of dwelling on. This REPLACED an earlier "notice it but never explain it" version. A character reacting to the grave IS the reveal, so there are no in-character reactions to write, and evasion is as bad as curiosity. Slot 8's lighting tells the story to GUESTS only.
- **Jasper and Edgar take the current storm stage as an explicit input** (`stormStageRegister` on each): presence decides WHETHER they speak, the stage decides HOW. Distant = casual bickering → Building → Active = rising tension → Grand Ritual = genuine panic (Edgar nearly silent; his silence is the loudest signal in the show) → Calm = de-escalating, humour returning. This is the mechanism behind Edgar's `arcDepth` — the comedy fades by stage, not on a timer. Keys match the engine's stage names exactly.
- **Evelina candy-giving** (`characters.evelina.candyGiving`): a warm, transactional trade for a guest's time, because any visitor might unknowingly carry what she needs. BRISK with small trick-or-treaters (arrive, trade, go — never hold or quiz them), extendable with guests who linger. The host physically hands out the candy; Evelina only narrates the trade and never directs anyone physically.
...
- **Full integration spec lives at `docs/lily-integration-spec.md`** (the master build doc: Part 1 pending items + Part 2 Lily spec ported from TheDadTech/UltraSkellyAdvanced). Read it before any Lily work. Key gates: Lily's BLE build starts only AFTER the Linux box is up AND the first full show run-through has passed.
- **Pre-hardware groundwork BUILT**: `lily/protocol.js` (UUIDs, CRC-8 Dallas/Maxim — check value 0xA1 over "123456789" — command framing, all command builders, movement-probe helpers; payload byte layouts are needs-verification-on-real-Lily) with unit tests in `lily/protocol.test.js` (`node lily/protocol.test.js`); `lily/mockLily.js` (SimulatedSkelly-pattern mock carrying the EXACT safety-interlock contract: disarmed by default, setMovement rejects while disarmed, disarm/disconnect force movement to none — the real noble adapter must match this surface). `settings.lilyEnabled` (default false) is the graceful-degradation flag: agents skip Lily cleanly when false/unreachable, she is never a hard dependency of the show.
- **Lily has NO eye-icon display** (eyes are mechanical pan/tilt+blink) — `setEyeIcon` is Ultra-Skelly-only; mood/reaction is conveyed entirely via the lantern (calm / wary / protective-ward states, spec §2.1.5). Jaw is believed audio-amplitude-reactive through her own speaker (verify on hardware); sink volume is therefore set to unity gain 1.0 on route.
- **noble compatibility smoke test (2026-09, Node 22/Linux container)**: `@abandonware/noble` installs and its native module COMPILES clean; runtime needs a real BlueZ host (container has no AF_BLUETOOTH). Native binding is platform-compiled, so when noble becomes a real dependency the OptiPlex needs a one-time `npm rebuild @abandonware/noble` on first setup — the ONLY sanctioned exception to the "never npm install on the show machine" rule; commit the Linux-built result after.
...
- Onkyo receiver reserved at **192.168.1.160**; Dell reserved at **192.168.1.168**. Govee light IPs still need entering in Test → System.
